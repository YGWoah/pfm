...
const CategoryContext = createContext<{
	...
	setChosenCategory: React.Dispatch<React.SetStateAction<CategoryType | null>>;
} | null>(null);

const Category = (category: { category: { nom: String; id: number } }) => {
	...
	return (
		<div
			...
			onClick={() => setChosenCategory?.setChosenCategory(category.category)}
		>
			<span
				style={{
					display: 'flex',
			...
			></span>
			<p className="font-medium inline float-left  ">{category.category.nom}</p>
		</div>
	);
};
...
const AddArticle = () => {
	...
	const [chosenCategory, setChosenCategory] = useState<CategoryType | null>(
		null
	);
	const getCategories = () => {
		try {
			...
			<div className="container flex flex-col mx-auto space-y-12 ng-untouched ng-pristine ng-valid">
				<fieldset className=" p-6 rounded-md shadow-sm ">
					<div className="grid grid-cols-2 gap-4 lg:col-span-3">
						...
						<div className="">
							<label htmlFor="website" className="font-medium text-lg">
								Image
							</label>
							<input
								id="Image Link"
